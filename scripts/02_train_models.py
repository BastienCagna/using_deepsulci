"""
    ...
    python 02_train_models.py -c pclean50A_hemi-L -m unet3d_d00b01_active --cuda 3 --dropout 0 --lr 0.0025 --momentum 0.9 --active --amount 3 --init 4 -r 2
"""
import os.path as op
from deepsulci.sulci_labeling.capsul.training import SulciDeepTraining, \
    SulciActiveDeepTraining
import json
...
def train_cohort(cohort, out_dir, fname, dropout=0, lr=0, momentum=0,
                 ...
                 inclusion_amount=1, max_iter=-1,
                 inclusion_strategy="proba_avg", test_cohort=None,
                 resampling=.5, detailed_log=False):
    ...
    if active_mode:
        ...
        for idx in indexes[:initial_amount]:
            init_graphs.append(graphs.pop(idx))
    ...
    proc.resampling_ratio = resampling
...
def main():
    ...
    parser.add_argument('--resampling', dest='resampling', type=float,
                        default=0.5, help='Resampling factor')
    ...
    for cohort in cohorts:
        ...
        for r in range(args.runs):
            ...
            if args.purge:
                print("Erasing previous results for run-{:02d}".format(r+1))
            ...
            train_cohort(cohort, mdl_dir, fname, args.dropout, args.lr,
                         ...
                         args.amount, args.max_iter, args.strategy, test_cohort,
                         args.resampling, args.detail)
    return None
